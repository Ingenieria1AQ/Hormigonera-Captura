...
Public Class frmConfiguracion_Andina
    ...
    Private Sub frmConexion_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            chBox_Hab_Imp.Checked = Convert.ToBoolean(Funciones.Obtener_Valor_Configuracion("UsaImpresora") = "1")
    ...
    Private Sub Btt_Conectar_Click(sender As Object, e As EventArgs) Handles Btt_Conectar.Click
        Try
            ...
            Select Case TabControl1.SelectedTab.Name
                Case "TP_Tolv1"
                    NombreCom = IIf(IsNothing(cboSerialPort1.SelectedValue), "", cboSerialPort1.SelectedValue)
                    Baud = Txt_Baud1.Text
                    ...
                    idTolva = CInt(dtConfigTol.Rows(0).Item("id")) 'ID PIEDRA
                Case "TP_Tolv2"
                    NombreCom = IIf(IsNothing(cboSerialPort2.SelectedValue), "", cboSerialPort2.SelectedValue)
                    Baud = Txt_Baud2.Text
                    ...
                    idTolva = CInt(dtConfigTol.Rows(1).Item("id")) 'ID ARENA

                Case "TP_Cemento"
                    NombreCom = IIf(IsNothing(cboSerialPort3.SelectedValue), "", cboSerialPort3.SelectedValue)
                    Baud = Txt_Baud3.Text
                    ...
                    idTolva = CInt(dtConfigTol.Rows(2).Item("id")) 'ID CEMENTO
            ...
            Using conection As New OleDbConnection(sConnString)
                Using cmd As New OleDbCommand
                    ...
                    Select Case Variables.tipoBD
                        Case "ACCESS"
                            cmd.CommandText = "SELECT * FROM NumeroTolvasTanques WHERE usaSerial=true ORDER BY id"
                        Case "SQLSERVER"
                            cmd.CommandText = "SELECT * FROM NumeroTolvasTanques WHERE usaSerial=1 ORDER BY id"
                    End Select
                    Using da As New OleDbDataAdapter(cmd)
                        da.Fill(dt)
                    End Using
